...
chull <- ggplot(data = states, fill = "white") +
  geom_sf() +
  geom_sf(data = grid_data, aes(col = slf_established), alpha = 0.5) +
...
# save the polygon
st_write(hull, file.path(here(), "figures", "GIS", "chull.shp"), driver = "ESRI Shapefile")
```
...
ggplot(data = states, fill = "white") +
  geom_sf() +
  geom_sf(data = grid_data, alpha = 0.5) +
...
grid_transports %<>% rename(bio_year = bio_yer,
                            latitude_rounded = lttd_rn,
                            longitude_rounded = lngtd_r,
                            slf_present = slf_prs,
                            slf_established = slf_stb,
                            DistToIntro = DstTInt)
head(grid_transports)
...
# Boxplot
outbreaks <- ggplot(jumps_compare_long, aes(y = DistanceValue/1000, x = Outbreaks)) + 
...
wilcox.test(DistToThreshold ~ Outbreaks, data = jumps_compare, paired = F)
# W = 167, p-value = 0.865

t.test(DistToRail ~ Outbreaks, data = jumps_compare)
wilcox.test(DistToRail ~ Outbreaks, data = jumps_compare, paired = F)
# W = 184, p-value = 0.486

t.test(DistToRoad ~ Outbreaks, data = jumps_compare)
wilcox.test(DistToRoad ~ Outbreaks, data = jumps_compare, paired = F)
# W = 170, p-value = 0.7928

t.test(DistIntRlRd ~ Outbreaks, data = jumps_compare)
wilcox.test(DistIntRlRd ~ Outbreaks, data = jumps_compare, paired = F)
# W = 168, p-value = 0.8408
```