...
dist <- past_jumps[which(names(past_jumps) == 'DistIntRlRd')]
risk = 0.95
threshold <- quantile(dist[1], probs = risk, na.rm = T)

buffer <- st_buffer(roadrail_intersect_zone, dist = threshold)
st_area(buffer)/sum(st_area(US))
175196000000/sum(st_area(US))
202499000000/sum(st_area(US))
...
leaflet(data = buffer) %>% 
  addTiles() %>% 
  addPolygons() %>%
  addCircles(data = jumps2020, ~longitude_rounded, ~latitude_rounded, 
