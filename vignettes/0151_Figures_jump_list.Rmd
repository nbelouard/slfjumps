...
map_rarified <- ggplot(data = grid_data) +
  ...
  geom_point(data = Jumps,
             aes(x = longitude_rounded, y = latitude_rounded, 
                 col = as.factor(bio_year), group = Rarefied, stroke = Rarefied, shape = Rarefied), size = 4) +
  ...
  theme(legend.position="right", text = element_text(size = 10),
   ...
   guides(shape = guide_legend("Dataset"), stroke = guide_legend("Dataset"), colour = guide_legend("Biological year"), alpha = guide_legend("Dataset"), fill = guide_legend("Biological year"))

map_rarified

ggsave(file.path(here(), "figures", "manuscript figures", "3. jumps_map.jpg"), map_rarified, height = 8, width = 8)
```
...
jumps_plot <- ggplot() +
  geom_bar(data = Jumps_year, aes(x = bio_year, y = n, fill = as.factor(bio_year), group = Rarefied, col = as.factor(bio_year), alpha = Rarefied), stat = "identity", lwd = .25) +
  scale_fill_manual(values = c("gold2", "firebrick3", "#0072B2", "#009E73")) +
...
ggsave(file.path(here(), "figures", "manuscript figures", "3. number of jumps.jpg"), jumps_plot, height = 2.5, width = 4)
```
...
Jumps_sectors %<>% select(-c(theta.1, sectors)) %>%
...
ggsave(file.path(here(), "figures", "manuscript figures", "3. jump distances.jpg"), MeanDist_jump, height = 2.5, width = 4)
```
...
ggsave(file.path(here(), "figures", "manuscript figures", "3. jump description.jpg"), fig2, height = 10, width = 10)
