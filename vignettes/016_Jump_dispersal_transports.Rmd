...
for (i in 1:10000){
  ...
  ModelsRail <- rbind(ModelsRail, 
                      c(summary_modRailac$coefficients[c(1:3, 10:12)], #instead of 1:4, 12:16 
  ...
  ModelsRoad <- rbind(ModelsRoad, 
                      c(summary_modRoadac$coefficients[c(1:3, 10:12)],  #instead of 1:4, 12:16 
  ...
  ModelsInt <- rbind(ModelsInt, 
                     c(summary_modIntac$coefficients[c(1:3, 10:12)],  #instead of 1:4, 12:16
...
sd(ModelsRail$`Adj.Rsquared`) #sd = 0.03
R2/(1-R2) #0.30
...
# Road
R2 = mean(ModelsRoad$`Adj.Rsquared`) #R2 = 0.05
sd(ModelsRoad$`Adj.Rsquared`) #sd = 0.02
R2/(1-R2) #0.055

pwr.f2.test(u = 3, 
            v = 401, 
            f2 = 0.055, #effect size
            sig.level = 0.05, #significance level
            power = NULL)
# power = 0.98
...
# Int
R2 = mean(ModelsInt$`Adj.Rsquared`) #R2 = 0.20
sd(ModelsInt$`Adj.Rsquared`) #sd = 0.03
R2/(1-R2) #0.25
...
for (i in 1:10000){
  ...
  summary_modIntac <- summary(modInt_ac)
  ModelsInt <- rbind(ModelsInt, 
                     c(summary_modIntac$coefficients[c(1:3, 10:12)], summary_modIntac$r.squared, #instead of 1:4, 12:16
                       summary_modIntac$adj.r.squared))
  #estimates: intercept, diffusers-jumpers, diffusers-negatives, autocorrelation coef; p-values: diffusers-jumpers, diffusers-negatives, autocorrelation coef
...
names(ModelsRail) = c("Int_est", "DJ_est", #"DN_est", 
                      ...
                      "Intp", "DJp", #"DNp", 
                      "ACp", "Rsquared", "Adj Rsquared")
write.csv(ModelsRail, file.path(here(), "tables", "ModelsRail_rarefied_pos.csv"), row.names = F)
...
names(ModelsRoad) = c("Int_est", "DJ_est", #"DN_est", 
                      ...
                      "Intp", "DJp", #"DNp", 
                      "ACp", "Rsquared", "Adj Rsquared")
write.csv(ModelsRoad, file.path(here(), "tables", "ModelsRoad_rarefied_pos.csv"), row.names = F)
...
names(ModelsInt) = c("Int_est", "DJ_est", #"DN_est", 
                     "AC_est",
                      "Intp", "DJp", #"DNp", 
                     "ACp", "Rsquared", "Adj Rsquared")
write.csv(ModelsInt, file.path(here(), "tables", "ModelsInt_rarefied_pos.csv"), row.names = F)
```
...
# Rail
R2 = mean(ModelsRail$`Adj Rsquared`) #R2 = 0.14
sd(ModelsRail$`Adj Rsquared`) #sd = 0.05
R2/(1-R2) #0.16
...
# Road
R2 = mean(ModelsRoad$`Adj Rsquared`) #R2 = 0.04
sd(ModelsRoad$`Adj Rsquared`) #sd = 0.04
R2/(1-R2) #0.04
...
# Int
R2 = mean(ModelsInt$`Adj Rsquared`) #R2 = 0.15
sd(ModelsInt$`Adj Rsquared`) #sd = 0.05
R2/(1-R2) #0.18
...
Statistical test (not paired because of multiple measures...)
...
Is there a difference between non-outbreaks and all points in outbreaks?
```{r statistical test all outbreaks vs non-outbreaks}

# DistToThreshold
bartlett.test(DistToThreshold ~ Dataset,  data = test_outbreaks %>% filter(Dataset %in% c("Outbreaks, all points", "Non-outbreaks")))
# p = 0.84, no diff in variance
t.test(DistToThreshold ~ Dataset, data = test_outbreaks %>% filter(Dataset %in% c("Outbreaks, all points", "Non-outbreaks")))
# p = 0.76, no diff


# DistToRail
bartlett.test(DistToRail ~ Dataset,  data = test_outbreaks %>% filter(Dataset %in% c("Outbreaks, all points", "Non-outbreaks")))
# p = 0.88, no diff in variance
t.test(DistToRail ~ Dataset, data = test_outbreaks %>% filter(Dataset %in% c("Outbreaks, all points", "Non-outbreaks")))
# p = 0.21, no diff


# DistToRoad
bartlett.test(DistToRoad ~ Dataset,  data = test_outbreaks %>% filter(Dataset %in% c("Outbreaks, all points", "Non-outbreaks")))
# p = 0.001, diff in variance...
t.test(DistToRoad ~ Dataset, data = test_outbreaks %>% filter(Dataset %in% c("Outbreaks, all points", "Non-outbreaks")))
# p = 0.167, no diff


# DistToIntRlRd
bartlett.test(DistIntRlRd ~ Dataset,  data = test_outbreaks %>% filter(Dataset %in% c("Outbreaks, all points", "Non-outbreaks")))
# p = 0.7076, no diff in variance
t.test(DistIntRlRd ~ Dataset, data = test_outbreaks %>% filter(Dataset %in% c("Outbreaks, all points", "Non-outbreaks")))
# p = 0.2269, no diff
```
