...
Highrisk %<>% mutate(type = recode(category,
                                  "fedex" = "package carriers", 
                                  "ups" = "package carriers",
                                  "amazonFullfilment" = "package carriers", 
                                  "landscape" = "natural products", 
                                  "lumberYards" = "natural products", 
                                  "sawmill" = "natural products",
...
# Package carriers
packages <- Highrisk_proj %>% filter(type == "package carriers")
...
# Naturalprod
naturalprod <- Highrisk_proj %>% filter(type == "natural products")
dim(naturalprod)
...
st_write(garages_buffer, file.path(here(), "exported-data", "GIS", "garages_buffer.shp"), append = F)
dim(garages)
st_crs(garages)
...
portsPA <- st_read(file.path(here(), "exported-data", "GIS", "portsPA.shp"), 
                quiet = T)
garages_buffer <- st_read(file.path(here(), "exported-data", "GIS", "garages_buffer.shp"), 
                quiet = T)
...
# Create rows for distances
slfPA_unique %<>% add_column(DistToPackages = NA,
                      DistToWood = NA,
                      DistToWineries = NA,
                      DistToHobbies = NA,
                      DistToGarages = NA,
...
for (j in 1:length(slfPA_unique$DistToPackages)){ 
  ...
  dist_wood <- st_distance(x = slfPA_unique[j,], y = naturalprod_buffer)
...
2. Reassemble in one long dataset with one column for the type of dataset
...
slfPA %>% filter(DistToGarages == Inf | is.na(DistToGarages))
# slfPA %>% filter(DistToBoats == Inf | is.na(DistToBoats))
# slfPA %>% filter(DistToOthers == Inf | is.na(DistToOthers))
...
dim(slfPA %>% filter(Category == "Jumpers"))[1] == dim(jumpsPA)[1]
slfPA %<>% rename(Category_full = Category)
head(slfPA)

slfPA %>% group_by(Category_full) %>% count()
```
...
slf_observed$Category_rarefied <- factor(slf_observed$Category_rarefied, 
                                         levels = c("Negatives", "Diffusers", 
                                                    "Jumpers"))
slf_observed$Category_full <- factor(slf_observed$Category_full, 
                                     levels = c("Negatives", "Diffusers", 
                                                "Jumpers"))

# Translate factors into ordinal
slf_uptodate <- slf_observed %>% 
  mutate(Category_rare_num = as.numeric(Category_rarefied),
  ...
  mutate(Category_rare = recode(Category_rare_max, "1" = "Negatives", 
                                "2" = "Diffusers", "3" = "Jumpers"),
         Category_full = recode(Category_full_max, "1" = "Negatives", 
                                "2" = "Diffusers", "3" = "Jumpers")) %>% 
...
# Order the Category levels
unique(slf_observedPA$Category_full)
slf_observedPA$Category_full <- factor(slf_observedPA$Category_full, 
                                     levels = c("Negatives", "Diffusers", 
                                                "Jumpers"))

# Remove unused columns
slf_observedPA %<>% select(bio_year, latitude_rounded, longitude_rounded, slf_established, 
                           DistToPackages, DistToWood, DistToWineries,
                           DistToHobbies, DistToGarages, DistToAirports, DistToPorts,
                           Category_full)
