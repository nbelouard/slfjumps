...
/!\ UPDATE: It may not be wise to round coordinates, because then distances to infrastructures are going to be biased. Let's see the number of jumps obtained if we don't round coordinates.

```{r rounding coordinates, echo = params$display, warnings = FALSE, message = FALSE, eval = params$run}

slf_tiny <- read.csv(file.path(here(), "exported-data", "slftiny.csv"), header = T)
head(slf_tiny)
...
#if grid size is 0.1 (100m), the number of decimal places is 4 at the smallest so this is still okay
...
grid_data <- slf_tiny %>%
  # mutate(latitude_grid = RoundTo(latitude, multiple = size_of_grid/111),
         # longitude_grid = RoundTo(longitude, multiple = size_of_grid/85)) %>%
  mutate(latitude_rounded = round(latitude, 5),
...
# dim(slf_tiny %>% filter(slf_established == T))[1] #20457
# Percentage of positive surveys
dim(grid_data %>% filter(slf_established == T))[1]/dim(grid_data)[1] #12.08%
# dim(slf_tiny %>% filter(slf_established == T))[1]/dim(slf_tiny)[1] #6.31%
```
...
<<<<<<< HEAD:vignettes/015_Jump_dispersal_list.Rmd
The area surveyed is huge, but in the western USA, mostly made of negative surveys 
we don't need to keep these areas because they are not useful to model anything!
#### UPDATE: I don't think this chunk is necessary!
...
>>>>>>> c3e2685d4fedcd60a8ecd575a7df27566f0c215c:vignettes/010_Jump_list.Rmd
...
<<<<<<< HEAD:vignettes/015_Jump_dispersal_list.Rmd
#Coordinates of the introduction site, extracted from Barringer et al. 2015
# centroid2 <- data.frame(longitude_rounded = -75.675340, 
                        # latitude_rounded = 40.415240) #introduction point
centroid2 <- data.frame(longitude = -75.675340, 
                        latitude = 40.415240) #introduction point

# centroid <- c(grid_data %>% filter(slf_established == T) %>% 
                # summarise(mean(longitude_rounded)) %>% 
                # pull(),
              # grid_data %>% filter(slf_established == T) %>% 
                # summarise(mean(latitude_rounded)) %>% 
                # pull()) # centroid (very close!)
...
tic()
grid_data <- grid_data %>%
  mutate(DistToIntro = distm(grid_data[,c(3,2)], centroid2, fun=distGeo)/1000)
toc()
# grid_data <- slf_tiny %>% 
  # mutate(DistToIntro = geosphere::distm(slf_tiny[,c(5,4)], centroid2, fun=distGeo)/1000)
...
# write.csv(grid_data, file.path(here(), "exported-data", "grid_data.csv"), 
          # row.names = F)
write.csv(grid_data, file.path(here(), "exported-data", "grid_data_norounding.csv"), 
          row.names = F)
dim(grid_data)[1]
...
head(grid_data)
# grid_data %<>% rename(latitude_rounded = latitude,
...
# List of jumps
head(optim_list)
write.csv(optim_list, file.path(here(), "tables", "optim_list100m.csv"))
...
write.csv(optim_sum, file.path(here(), "tables", "optim_jumps100m.csv"))
...
ggsave(file.path(here(), "figures", "jump_list", "4.optim_plot100m.jpg"), 
       optim_plot)
```
...
write.csv(Results$Dist, file.path(here(), "exported-data", "thresholds100m.csv"), 
          row.names = F)
write.csv(Results$Jump, file.path(here(), "exported-data", "jumps100m.csv"), 
          row.names = F)
```
...
write.csv(Jump_groups, file.path(here(), "exported-data", "jump_groups100m.csv"), 
          row.names = F)
```
